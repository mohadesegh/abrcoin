...
const TitleReviewContainer = tw.div`flex flex-col sm:flex-row sm:justify-between sm:text-right`;
const Title = tw.h5`max-w-64 text-2xl font-bold`;
...
export default () => {
  ...
  const cards = [
    ...
    },
    {
      ...
      title: "پرونده شکایت مالباختگان دوج کوین از ایلان ماسک مختومه میشود/",
    },
    {
      ...
      title: "کیف پول سخت افزاری چیست؟چطور کیف پول سخت افزاری مناسب پیدا کنیم؟",
    },
  ]
  ...
  return (
    <Container>
      <Content>
        <HeadingWithControl>
          <Heading>آخرین اخبار و مقالات</Heading>
        </HeadingWithControl>
        <CardSlider ref={setSliderRef} {...sliderSettings}>
          {cards.map((card, index) => (
            <Card key={index}>
              ...
              <PrimaryButton onClick={handleClick}>مشاهده خبر</PrimaryButton>
            </Card>
          ))}
        </CardSlider>
      </Content>
